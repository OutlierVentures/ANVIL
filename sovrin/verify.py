...
async def request_proof_of_credential(verifier, prover, proof_request = {}):
    ...
    verifier['job_application_proof_request'] = proof_request
...
async def create_proof_of_credential(prover, verifier, self_attested_attrs = {}, requested_attrs = [], requested_preds = [], non_issuer_attributes = []):
    ...
    for i in range(1, num_attributes_to_search + 1):
        stri = str(i)
        cred_attrs['cred_for_attr' + stri] = await get_credential_for_referent(search_for_job_application_proof_request, 'attr' + stri + '_referent')
    cred_predicates = {}
    for i in range(1, num_predicates + 1):
        stri = str(i)
        cred_predicates['cred_for_predicate' + stri] = await get_credential_for_referent(search_for_job_application_proof_request, 'predicate' + stri + '_referent')
    ...
    for _, value in cred_predicates.items():
        creds_for_proof[value['referent']] = value
    prover['creds_for_job_application_proof'] = creds_for_proof
    ...
    prover['job_application_requested_creds'] = proof_request_reply_from_prover
...
async def verify_proof(verifier, assertions_to_make):
    ...
    for key, value in assertions_to_make['revealed'].items():
        assert value == decrypted_job_application_proof['requested_proof']['revealed_attrs'][key]['raw']
    for key, value in assertions_to_make['self_attested'].items():
        assert value == decrypted_job_application_proof['requested_proof']['self_attested_attrs'][key]
