...
async def receive_credential_offer(prover):
    ...
    authdecrypted_certificate_cred_offer = json.loads(json_cred_offer['cred_offer'])
    prover[prover['unique_schema_name'] + '_schema_id'] = authdecrypted_certificate_cred_offer['schema_id']
    prover[prover['unique_schema_name'] + '_cred_def_id'] = authdecrypted_certificate_cred_offer['cred_def_id']
...
async def request_credential(prover, values):
    ...
    (prover[prover['unique_schema_name'] + '_cred_request'], prover[prover['unique_schema_name'] + '_cred_request_metadata']) = \
        await anoncreds.prover_create_credential_req(prover['wallet'], prover['issuer_did'],
                                                     prover[prover['unique_schema_name'] + '_cred_offer'], prover['issuer_certificate_cred_def'],
    ...
    prover['authcrypted_certificate_cred_request'] = \
        await crypto.auth_crypt(prover['wallet'], prover['issuer_key'], prover['issuer_key_for_prover'],
                                prover[prover['unique_schema_name'] + '_cred_request'].encode('utf-8'))
    return prover
...
async def create_and_send_credential(issuer):
    ...
    # Decrypt
    issuer['prover_key_for_issuer'], issuer[issuer['unique_schema_name'] + '_cred_request'], _ = \
